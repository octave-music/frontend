...
interface MobilePlayerExpandedMainControlsProps {
  ...
  onTogglePlay: () => void;
  onSkipTrack: () => void; // This should be the one handling single/double click logic passed from parent
  onPreviousTrack: () => void; // This should be the one handling single/double click logic passed from parent
  onShuffleQueue: () => void;
...
const MobilePlayerExpandedMainControls: React.FC<MobilePlayerExpandedMainControlsProps> = ({
  ...
  return (
    <div className="w-full flex items-center justify-around mb-6 p-2 shrink-0">
      <button
        onClick={onShuffleQueue}
        className={`p-3 rounded-full transition-colors ${shuffleOn ? "text-purple-400" : "text-white/60 hover:text-white/80 hover:bg-white/5"}`}
      ...
      >
        {isPlaying ? (
          ...
          <Play className="w-8 h-8 ml-1" /> // Slight offset for visual centering of play icon
        )}
      </motion.button>
      <button
        onClick={onSkipTrack}
      ...
      <button
        ...
        className={`p-3 rounded-full transition-colors ${repeatMode !== "off" ? "text-purple-400" : "text-white/60 hover:text-white/80 hover:bg-white/5"}`}
